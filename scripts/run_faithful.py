"""Tune and audit faithful CIFAR-10 unlearning baselines."""
import json
import sys
...
from topotrace.unlearn import neggrad_plus, scrub, ssd

METHODS = ("neggrad_plus", "scrub_tuned", "ssd_tuned")
...
def main():
    ...
    result_path = out / "faithful_baselines.json"
    ...
    grids = {
        "neggrad_plus": [dict(beta=beta, epochs=epochs)
                         for beta, epochs in product((.90, .95, .99), (1, 2))],
        "scrub_tuned": [dict(lr=lr, epochs=epochs, alpha=alpha)
                        for lr, epochs, alpha in product((1e-4, 5e-4), (2, 4),
                                                         (.5, 1.0))],
        "ssd_tuned": [dict(dampening_alpha=alpha, lam=lam)
                      for alpha, lam in product((5, 10, 30), (.5, 1.0))],
    }
    calls = {
        "neggrad_plus": lambda config: neggrad_plus(
            original, X, y, forget_idx, retain_idx, seed=2000, **config),
        "scrub_tuned": lambda config: scrub(
            original, X, y, forget_idx, retain_idx, seed=2000, **config),
        "ssd_tuned": lambda config: ssd(
            original, X, y, forget_idx, retain_idx, **config),
    }
    ...
    for method in METHODS:
        ...
        for config in grids[method]:
            ...
            rows.append(row)
            print(f"{method:14s} {str(config):58s} retain={row['retain']:.4f} "
                  f"forget={row['forget']:.4f}", flush=True)
        eligible = [row for row in rows if row["eligible"]]
        if not eligible:
            raise RuntimeError(f"no eligible {method} configuration")
    ...
    for method in METHODS:
        ...
        print(f"{method:14s} {str(row['config']):58s} retain={row['retain']:.4f} "
              f"forget={row['forget']:.4f} error={row['target_error']:.4f}",
              flush=True)
    result_path.write_text(json.dumps(
        {"target_forget_accuracy": TARGET, "tuning": tuning}, indent=2))

    embeddings, accs, mia = {}, {method: [] for method in METHODS}, {
    ...
    for seed in range(10):
        ...
        for method in METHODS:
            ...
            if method == "neggrad_plus":
                model = neggrad_plus(original, X, y, forget_idx, retain_idx,
                                     seed=2000 + seed, **config)
            elif method == "scrub_tuned":
                model = scrub(original, X, y, forget_idx, retain_idx,
                              seed=2000 + seed, **config)
            else:
                model = ssd(original, X, y, forget_idx, retain_idx, **config)
            ...
            print(f"  {method:14s} retain={values['retain']:.4f} "
                  f"forget={values['forget']:.4f} test={values['test']:.4f} "
                  f"MIA={mia[method][-1]:.4f}", flush=True)
    np.savez_compressed(out / "embeddings_faithful.npz", **embeddings)
